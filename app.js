...
function checkAnswer() {
  ...
  state.answered = true;
  state.lastResult = { isCorrect: isCorrect, userAnswer: userAnswer };
  if (!isCorrect) {
...
function renderResult(q, isCorrect, userAnswer) {
  ...
  if (!isCorrect && userAnswer) {
    const u = userAnswer.split("").map(function(l) { return l + ". " + (q.options[l] || ""); }).join("；");
  ...
  if (!isCorrect && userAnswer) {
    userAnswer.split("").forEach(function(l) {
      const x = document.querySelector("[data-opt=\"" + l + "\"]");
...
function updateStats(q) {
  ...
  e.innerHTML = "<span>第 " + cur + " / " + total + " 题" + extra + "</span><span style=\"font-size:12px;color:var(--text-light);\">" + getAccuracy() + "</span>";
}
...
function renderCard(q) {
  ...
  c.innerHTML = "<div class=\"card\" id=\"questionCard\"><div class=\"card-header\"><div class=\"question-meta\"><span class=\"question-number\">第 " + q.id + " 题</span><span class=\"badge " + bc + "\">" + q.type + "</span></div><div class=\"wrong-count\"><span><span class=\"count-value\" style=\"font-weight:600;color:var(--wrong)\">" + wc + "</span> 次错误</span><button class=\"btn-icon\" onclick=\"window.decrementWrong(" + q.id + ")\" title=\"减少错误次数\">-</button><button class=\"btn-icon\" onclick=\"window.incrementWrong(" + q.id + ")\" title=\"增加错误次数\">+</button></div></div><div class=\"question-body\"><p class=\"question-text\">" + escapeHtml(q.question) + "</p></div>" + oh + jh + "<div class=\"submit-area\"><button class=\"btn-primary\" id=\"submitBtn\" onclick=\"window.checkAnswer()\">提交答案</button></div><div class=\"result\" id=\"result\" style=\"display:none\"></div><div class=\"notes-section\"><button class=\"notes-toggle\" onclick=\"window.toggleNotes()\"><div class=\"notes-toggle-left\"><svg xmlns=\"http://www.w3.org/2000/svg\" width=\"16\" height=\"16\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z\"/></svg><span>备注</span></div><div style=\"display:flex;align-items:center;gap:6px;\"><span class=\"notes-indicator " + (notes ? "has-notes" : "no-notes") + "\"></span><svg class=\"chevron\" xmlns=\"http://www.w3.org/2000/svg\" width=\"16\" height=\"16\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m6 9 6 6 6-6\"/></svg></div></button><div class=\"notes-content\" id=\"notesContent\" style=\"display:none\"><textarea id=\"notesTextarea\" placeholder=\"在此写下备注...\" oninput=\"window.saveCurrentNote()\">" + escapeHtml(notes) + "</textarea></div></div></div>";
...
function renderOptions(q) {
  ...
  for (let i = 0; i < k.length; i++) {
    ...
    html += "<label class=\"option\" data-opt=\"" + l + "\" onclick=\"window.selectOption('" + l + "'," + isMulti + ")\"><input type=\"" + it + "\" name=\"" + nm + "\" value=\"" + l + "\"><span class=\"option-marker " + mc + "\">" + l + "</span><span class=\"option-text\">" + escapeHtml(t) + "</span></label>";
  }
  return html;
}
...
function renderJudgeOptions(q) {
  ...
  for (let i = 0; i < k.length; i++) {
    const l = k[i];
    const t = q.options[l];
    html += "<button class=\"judge-btn\" data-value=\"" + l + "\" onclick=\"window.selectJudgeOption('" + l + "')\">" + escapeHtml(t) + "</button>";
  }
  return html;
}
...
function renderNav() {
  ...
  document.getElementById("navBar").innerHTML = "" +
    "<button class=\"btn-nav\" onclick=\"window.goPrev()\" " + (cur === 0 ? "disabled" : "") + ">" +
    ...
    "</button>" +
    "<button class=\"btn-nav\" onclick=\"window.goNext()\" " + (cur >= total - 1 ? "disabled" : "") + ">" +
